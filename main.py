...
def prepare_eval_train(eval_train):
    ...
    action_priors = pd.DataFrame(np.vstack(eval_df[2]))
    values = pd.DataFrame(np.vstack(eval_df[1]))
    return states, [action_priors, values]
...
def plot_test(evaluator_network, forecast_network, LOGFILE=True):
    ...
    cum_r = 0
    for i in range(960):
        action, root = UCT_search(StateNode(state, env.time), SEARCH_DEPTH, evaluator=evaluator_network, forecaster=forecast_network)
...
if __name__ == "__main__":
    ...
    if (episode+1) % PRINT_EVERY_X_ITER:
